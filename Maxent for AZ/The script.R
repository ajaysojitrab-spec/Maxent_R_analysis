...
# --- Calculate TSS and Omission Rate ---

# Extract predicted suitability for the test set
test_pred <- predict(maxent_model, test_data[, vars_common])

# True presence/absence labels in test set
test_obs <- test_data$presence

# Select threshold maximizing sum of sensitivity and specificity using dismo::evaluate results
max_sens_spec_index <- which.max(eval_results@TPR + eval_results@TNR)
threshold <- eval_results@t[max_sens_spec_index]

# Convert predicted suitability to binary presence/absence using threshold
test_pred_binary <- ifelse(test_pred >= threshold, 1, 0)

# Calculate confusion matrix components
TP <- sum(test_pred_binary == 1 & test_obs == 1)
TN <- sum(test_pred_binary == 0 & test_obs == 0)
FP <- sum(test_pred_binary == 1 & test_obs == 0)
FN <- sum(test_pred_binary == 0 & test_obs == 1)
...
# Omission Rate (proportion of presences omitted)
...
cat("True Skill Statistic (TSS):", round(TSS, 4), "\n")
cat("Omission Rate:", round(omission_rate, 4), "\n")

# Prepare metrics data frame
metrics <- data.frame(
  Metric = c("AUC", "TSS", "Sensitivity", "Specificity", "Omission Rate"),
  Value = c(eval_results@auc, TSS, sensitivity, specificity, omission_rate)
)

# Save metrics to CSV
metrics_csv_path <- file.path(output_dir, "model_evaluation_metrics.csv")
write.csv(metrics, metrics_csv_path, row.names = FALSE)
cat("Evaluation metrics saved to:", metrics_csv_path, "\n")

# Prepare radar chart data (repeat first row to close polygon)
metrics_radar <- rbind(metrics, metrics[1, ])
...
fig <- plot_ly(
  ...
  marker = list(size = 12, color = 'red', line = list(width = 2, color = 'white'))
) %>%
  layout(
    polar = list(
      radialaxis = list(visible = TRUE, range = c(0, 1))
    ),
    title = "Model Evaluation Metrics Radar Chart"
  )

# Save interactive radar chart as HTML
radar_html_path <- file.path(output_dir, "model_evaluation_radar_chart.html")
htmlwidgets::saveWidget(fig, radar_html_path)
cat("Radar chart saved as HTML to:", radar_html_path, "\n")
